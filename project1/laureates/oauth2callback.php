...
$redirect_uri = "https://node36.webte.fei.stuba.sk/test2/oauth2callback.php"; // Redirect URI for the OAuth2 callback. Must match the one in the Google Cloud Console.
...
if (isset($_GET['code'])) {
    ...
    $_SESSION["id"] = $userInfo->getId(); // ID пользователя
    $_SESSION["fullname"] = $userInfo->getName(); // Имя пользователя
    $_SESSION["email"] = $userInfo->getEmail(); // Email пользователя

    // TODO: Implement a mechanism to save login information - user_id, login_type, email, fullname - to database.

    $redirect_uri = 'https://node36.webte.fei.stuba.sk/test2/restricted.php'; // Redirect to the restricted page or dashboard.
